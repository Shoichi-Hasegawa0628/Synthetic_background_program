...
if __name__ == '__main__':
    root = Path.home().joinpath("HSR/datasets/rgiro2022_object")
    objects_dir = root.joinpath("objects")
    background_dir_dir = root.joinpath("backgrounds")

    save_root_dir = Path.home().joinpath("HSR/datasets/rgiro2022_object_for_yolov5")
    if save_root_dir.exists():
        shutil.rmtree(save_root_dir)
    save_root_dir.mkdir(parents=True)

    dataset = YCBDataset(objects_dir, background_dir_dir, scale1=(0.44, 0.88), scale2=(0.44, 0.88))
    ...
    dataset = YCBDataset(objects_dir, background_dir_dir, scale1=(0.4, 0.88), scale2=(0.4, 0.88))
    root_train_dir = save_root_dir.joinpath("train")
    root_train_dir.mkdir()
    save_data(root_train_dir, dataset, iter=20)
